...
const EditModal = ({modalHeading,designId,designName}) => {
    ...
    const handleSubmit=async(e)=>{
        ...
        if(user && user.admin){ 
            if(user.expiresIn>d){
                ...
                fetch('http://localhost:5000/api/edit/'+user.token,{
                method:'POST',
                credentials:'include',
                body:formData
                }).then(res=>res.json())
    ...
    return ( 
        <div className="d-flex justify-content-start">
            <button className='addModal btn btn-secondary btn-sm'  data-bs-toggle="modal" data-bs-target="#myModal" onClick={()=>{setMsg(false)}}>
                Edit design
            </button>
